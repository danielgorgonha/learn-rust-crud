...
pub async fn read_all_data(req: Request<AppState>) -> tide::Result {
    ...
    info!(
        ...
        "Retrieved all records from state"
    );
    ...
    info!(
        ...
        "Read all data completed successfully"
    );
...
pub async fn read_data(req: Request<AppState>) -> tide::Result {
    ...
    if let Some(entry) = app_state.data.get(&id) {
        ...
        info!(
            ...
            record_id = %id,
            owner = %entry.owner,
            func_count = entry.func_names.len(),
            execution_time_ms = execution_time.as_millis(),
            "Read single data completed successfully"
        );
        ...
        info!(
            ...
            record_id = %id,
            execution_time_ms = execution_time.as_millis(),
            "Read single data failed - record not found"
        );
